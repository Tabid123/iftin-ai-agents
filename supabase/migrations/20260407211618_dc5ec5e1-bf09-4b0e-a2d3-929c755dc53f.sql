-- Fix timezone for all admin RPC functions to use Africa/Mogadishu (UTC+3)

-- 1. Fix get_admin_analytics_summary
...
BEGIN
  today_start := date_trunc('day', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
  week_start := date_trunc('week', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
  month_start := date_trunc('month', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
  year_start := date_trunc('year', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
...
-- 2. Fix get_admin_provider_daily_stats
CREATE OR REPLACE FUNCTION public.get_admin_provider_daily_stats(p_date date DEFAULT NULL)
 RETURNS json
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  use_date DATE;
BEGIN
  -- Default to today in Mogadishu timezone
  use_date := COALESCE(p_date, (now() AT TIME ZONE 'Africa/Mogadishu')::date);
  ...
  FROM (
    ...
    INNER JOIN orders_with_cost owc
     ...
     AND owc.created_at >= (use_date::text || ' 00:00:00')::timestamptz AT TIME ZONE 'Africa/Mogadishu' AT TIME ZONE 'UTC'
     AND owc.created_at < ((use_date + 1)::text || ' 00:00:00')::timestamptz AT TIME ZONE 'Africa/Mogadishu' AT TIME ZONE 'UTC'
...
-- 3. Fix get_admin_transactions_summary
...
BEGIN
  ...
  CASE p_period
    WHEN 'today' THEN date_from := date_trunc('day', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
    WHEN 'yesterday' THEN date_from := (date_trunc('day', now() AT TIME ZONE 'Africa/Mogadishu') - interval '1 day') AT TIME ZONE 'Africa/Mogadishu';
    WHEN 'week' THEN date_from := date_trunc('week', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
    WHEN 'month' THEN date_from := date_trunc('month', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
    WHEN 'year' THEN date_from := date_trunc('year', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
  ...
  today_start AS (
    SELECT date_trunc('day', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu' AS ts
  ),
  month_start AS (
    SELECT date_trunc('month', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu' AS ts
  )
  SELECT json_build_object(
    'transactions_today', COALESCE((SELECT COUNT(*) FROM base_orders WHERE created_at >= (SELECT ts FROM today_start) AND delivery_status = 'delivered'), 0),
    'sales_today', COALESCE((SELECT SUM(selling_price) FROM base_orders WHERE created_at >= (SELECT ts FROM today_start) AND delivery_status = 'delivered'), 0),
    'cost_today', COALESCE((SELECT SUM(effective_cost) FROM base_orders WHERE created_at >= (SELECT ts FROM today_start) AND delivery_status = 'delivered'), 0),
    'sales_this_month', COALESCE((SELECT SUM(selling_price) FROM base_orders WHERE created_at >= (SELECT ts FROM month_start) AND delivery_status = 'delivered'), 0),
    'cost_this_month', COALESCE((SELECT SUM(effective_cost) FROM base_orders WHERE created_at >= (SELECT ts FROM month_start) AND delivery_status = 'delivered'), 0),
...
-- 4. Fix get_admin_transactions_paginated
...
BEGIN
  ...
  CASE p_period
    WHEN 'today' THEN date_from := date_trunc('day', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu'; date_to := now();
    WHEN 'yesterday' THEN date_from := (date_trunc('day', now() AT TIME ZONE 'Africa/Mogadishu') - interval '1 day') AT TIME ZONE 'Africa/Mogadishu'; date_to := date_trunc('day', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu';
    WHEN 'week' THEN date_from := date_trunc('week', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu'; date_to := now();
    WHEN 'month' THEN date_from := date_trunc('month', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu'; date_to := now();
    WHEN 'year' THEN date_from := date_trunc('year', now() AT TIME ZONE 'Africa/Mogadishu') AT TIME ZONE 'Africa/Mogadishu'; date_to := now();
